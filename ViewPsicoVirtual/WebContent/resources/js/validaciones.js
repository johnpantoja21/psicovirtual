...
function setFormat(valorFormato)// define el formato a usar por la funcion
								// isDate(val,format);
{
...
/*
 * Metodo elaborado por Jorge Ivan Marmolejo Este metodo valida que la cadena
 * tenga solo letras
 */
...
function FechaValida(dia, mes, anio) {
	...
	if ((y2k(test.getYear()) == anio) && (mes == test.getMonth())
			&& (dia == test.getDate()))
		return true;
	else
		return false
...
function fecha() {
	...
	if (daym < 10)
		daym = "0" + daym
	var dayarray = new Array("Domingo", "Lunes", "Martes", "Miercoles",
...
// M�todo que permite Quitar los espacios en blanco a los lados en las formas
// que tiene campos de tipo texto
...
function clearFormSpace() {
	...
	for ( var i = 0; i < forma.elements.length; i++) {
		if (forma.elements[i].type == "text"
				|| forma.elements[i].type == "textArea") {
			forma.elements[i].value = Trim(forma.elements[i].value);
		}
	}
}
// M�todo que permite saber en un combo se selecciono una opcion si es -1 no
// se selecciono
// Hecho Por Diego Gomez diegomez@jvsoftware.com
...
/*
 * Metodo elaborado por Jorge Ivan Marmolejo Este metodo valida si una cadena es
 * numerica tipo entero y de signo positivo
 */
 ...
 * Elaborado por Jorge Hern�n V�lez Modificada por Alejandro Alvarez, para
...
function isNumericPositivo(str) {
	...
	return true;
	/*
	 * for (var i=0; i< str.length; i++) { if (str.charAt(i) == "-") return
	 * false; } return true;
	 */
}
/*
...
// Para Validar Solo Numeros y Rango : inRange ej.: inRange1:21 Lo anterior es
// Rango de 1 hasta 21
// Para Validar Solo Numeros y Consistencia Decimal : validarDecimales ej.:
// validarDecimales8:2 Lo anterior es validar que el decimal tenga 8 digitos
...
// Para validar fecha o periodo con un determinado formato: isDFormat ej:
// isDFormat:yyyy-MM
...
function ValidaForma() {
	...
	clearFormSpace();
	transformToUpperCase();// transforma los campos a
							// may�sculas,jmduran@jvsoftware.com
			...
			if ((val = val.value) != "") {
				...
				if (test.indexOf('isEmail') != -1) {
					...
					if (bandera == false) {
						errors += '- '
								+ nm
								+ ' debe contener una direcci�n de email v�lida.\n';
					}
					...
					if (bandera == false) {
						errors += '- '
								+ nm
								+ ' debe ser una fecha v�lida entre el a�o 1700 y 2300.\n';
					}
					...
					var bandera = isDateFormat(val, dateFormat);
					if (bandera == false) {
						errors += '- ' + nm + ' no tiene el formato '
								+ dateFormat.toUpperCase() + ' .\n';
					}
				} else if (test.indexOf('isMoney') != -1)// valida si el
															// formato es moneda
				{
					var bandera = validarMoneda(val);
					if (bandera == false) {
						errors += '- ' + nm
								+ ' debe ser un formato de moneda v�lido.\n';
					}
				}
				...
				else if (test.indexOf('isLetter') != -1)// JD
				{
					...
					if (swcheck == 0) {
						errors += '- ' + nm
								+ ' requiere seleccionar una opci�n.\n';
					}
				} else if (test.indexOf('isCombo') != -1) {
					if (val == "-1")
						errors += '- ' + nm + ' no se seleccion�.\n';

				} else if (test.indexOf('isEspecial') != -1
						|| test.indexOf('isNotNum') != -1) {
					if (test.indexOf('isEspecial') != -1) {
						if (!validarCaracteres(val))
							errors += '- '
									+ nm
									+ ' contiene caracteres con tildes o caracteres no v�lidos .\n';
					}
					...
					if (test.indexOf('isNotNum') != -1) {
						...
						if (!isNaN(num))
							errors += '- ' + nm
									+ ' NO debe contener solo n�meros.\n';
					}

				} else if (test.indexOf('isPassword') != -1) {
					if (!validarClave(val))
						errors += '- ' + nm
								+ ' contiene caracteres no v�lidos.\n';
					...
					if (!isNumericIntPositivo(val))
						errors += '- ' + nm
								+ ' debe ser n�mero entero positivo.\n';
				} else if (test.indexOf('isInteger') != -1) {
					if (!validarEnteros(val))
						errors += '- ' + nm
								+ ' debe contener n�meros enteros.\n';
				} else if (test.indexOf('isPositive') != -1) {
					if (!isNumericPositivo(val))
						errors += '- ' + nm
								+ ' debe contener n�meros positivos.\n';
				} else if (test.indexOf('validarDecimales') != -1)// Adicionado
																	// por
																	// Alejandro
																	// para
																	// validar
				...
				{
					p = test.indexOf(':');
					enteros = test.substring(16, p);// guardamos la cantidad de
													// enteros que permite
													// almacenar la base de
													// datos
					decimales = test.substring(p + 1);// guardamos la cantidad
														// de decimales que
					...
					if (isNumeric(val)) {
						...
						if (coma != -1 && punto < 0)
							errors += '- '
									+ nm
									+ ' debe separar su parte decimal con punto no con coma.\n';
						else if (punto != -1)// si vienen decimales validamos
												// que traiga la cantidad
												// correcta
						{
							var parteEntera = val.substring(0, punto);// guardamos
																		// la
																		// parte
																		// entera
							var parteDecimal = val.substring(punto + 1);// guardamos
																		// la
																		// parte
																		// decimal
							if (parteEntera.length > enteros) {
								errors += '- ' + nm
										+ ' debe contener un m�ximo de '
										+ enteros + ' digitos enteros y '
										+ decimales + ' decimales.\n';
							}
						...
						errors += '- ' + nm
								+ ' debe ser un n�mero con un m�ximo de '
								+ enteros + ' digitos enteros y ' + decimales
								+ ' decimales.\n';
				} else if (test != 'R') {
					num = parseFloat(val);
					if (isNaN(val))
					...
					if (test.indexOf('isMayor') != -1) {
						...
						if (num <= min)
							errors += '- ' + nm
									+ ' debe contener n�meros mayores a '
									+ min + '\n';
					}
					if (test.indexOf('inRange') != -1) {
						...
						if (num < min || max < num)
							errors += '- ' + nm
									+ ' debe contener solo n�meros entre '
									+ min + ' y ' + max + '.\n';
					}
				}
...
// ------------------------------------------------------------------
function isDate(val) {
	// si no recibe el formato lo toma de la variable global formato.Nota: Debe
	// inicializarse esta variabl�e a trav�s del metodo setFormat
	// Modificado por Alejandro Alvarez para validar que las fechas validas
	// esten unicamente entre el a�o 1900 y 2200.
...
// Funcion que coloca todos los campos de la forma que esten disabled o readonly
// con color de fondo gris
...
// Nota:funciona si la pagina no tiene el llamaado a la funcion onLoad en el
// body ej <body onLoad=funcion() >
	...
	while (document.forms[i]) {
		if (document.forms[i] != null) {
			for ( var j = 0; j < document.forms[i].elements.length; j++) {
				...
				// el resto de objetos solo tienen el atributo disabled no
				// tienen readonly
				else if (document.forms[i].elements[j].type == "select"
...
function aplicarFormatoTR(tr) {
	if (tr != null) {
		for ( var k = 0; k < tr.childNodes.length; k++) {
			...
			// el resto de objetos solo tienen el atributo disabled no tienen
			// readonly
			else if (elemento.type == "select" || elemento.type == "checkbox"
					|| elemento.type == "radio") {
...
// M�todo que permite deshabilitar o habilitar todos campos de tipo texto de
// una forma
...
// Funci�n que transforma los datos en May�sculas a aquellos campos que
// tengan la clase SnowInput, al resto no les hace nada
	...
	while (document.forms[i]) {
		if (document.forms[i] != null) {
			...
			for ( var j = 0; j < forma.elements.length; j++) {
				if (forma.elements[j].type != null
						&& (forma.elements[j].type == "text" || forma.elements[j].type
								.toLowerCase() == "textarea")) {
					if (forma.elements[j].className == 'SnowInput') {
						forma.elements[j].value = forma.elements[j].value
								.toUpperCase();
					}
				}
			}
 ...
 */
// Coloca la rura en el titulo de la ventana, esto para saber donde esta ubicado
// el usuario
if (parent == null)
	document.title = "Psicovirtual";// si es una
																// pagina sola
else if (parent.parent != null)
	parent.parent.document.title = "Psicovirtual";// si
																				// es
																				// una
																				// p�gina
																				// dentro
	...
	parent.document.title = "Psicovirtual";// si es
																		// una
																		// p�gina
																		// dentro
																		// de 1
																		// frame
	// Metodo encargado de validar cada vez que se cambie el valor del cupo
	// padre que sea numerico, y si es superior a la suma de los valores de los
	// cupos hijos
function validarValorCupoPadre(obj) {
	try {
		// Se determina si lo ingresado es numerico
		if (isNaN(parseFloat(convertirMonedaACadena(obj.value)))
				|| obj.value.length == 0) {
			alert('Por favor ingrese un valor valido.');
			obj.value = '';
			obj.focus;
		} else {
			// Se valida si la suma de los cupos hijos agregados es inferior o
			// igaul al valor del cupo padre
...
// Este metodo funciona para las pantallas de creacion y modificacion de cupos
// creditos
// Metodo encargado de validar el valor ingresado al momento de crear un cupo
// hijo.
	...
	try {
		...
		if (isNaN(parseFloat(convertirMonedaACadena(obj.value)))
			...
			if (objCupoPadre.value != '') {
				// Se valida si el valor del cupo credito hijo es mayor al valor
				// del cupo credito
				if (parseFloat(convertirMonedaACadena(obj.value)) > parseFloat(convertirMonedaACadena(objCupoPadre.value))) {
					...
					// Se valida el valor de los cupos hijos + el valor a
					// agregar es inferior al valor del cupo credito
					var valorCuposHijos = parseFloat(valoresCuposHijos());
					...
					if (parseFloat(valorCuposHijos) > parseFloat(convertirMonedaACadena(objCupoPadre.value))) {
						// alert("Valor cupo
						// hijo:"+parseFloat(valorCuposHijos));
						// alert("Valor cupo
						// padre:"+parseFloat(objCupoPadre.value));
...
function valoresCuposHijos() {
	...
	// Se valida si existen cupos hijos ya agregados y si el valor de los cupos
	// es superior al valor del cupo credito
	for ( var a = 0; a < 5000; a++) {
		try {
			valorCuposHijos += parseFloat(convertirMonedaACadena(document
					.getElementById('form1:page1:table1:' + a + ':outputText2').innerHTML));
...
function validarObligacion(error) {
	...
	var tipoObligacion = document
			.getElementById('form1:page1:selectOneChoice1');
	var moneda = document.getElementById('form1:page1:selectOneChoice2');
	var tipoPeriodicidadObligacion = document
			.getElementById('form1:page1:selectOneChoice4');
	var periodicidadObligacion = document
			.getElementById('form1:page1:selectOneChoice5');
	var periodicidadDiasObligacion = document
			.getElementById('form1:page1:inputText7');
	...
	// Si no se genero error se realizan las validaciones adicionales para la
	// obligacion.
	if (error) {
		...
		if (tipoPeriodicidadObligacion.value == 0
				|| tipoPeriodicidadObligacion.value == null) {
		...
		if (tipoPeriodicidadObligacion.value == 1) {
			// Se valida si selecciono alguna periodicidad valida para la
			// obligacion.
			if (periodicidadObligacion.value == 0
					|| periodicidadObligacion.value == null) {
			...
			if (tipoPeriodicidadObligacion.value == 2) {
				if (periodicidadDiasObligacion.value == 0
						|| periodicidadDiasObligacion.value.length == 0) {
					mensaje += "- Ingrese la cantidad de dias aociados a la periodicidad de la oblgacion.\n";
				}
			}
		}

		// Validacion para la parte de los intereses, si el tipo de obligacion
		// es obligacion.
...
function validarCalculoTasa() {
	...
	var tasaNominalVariable = document
			.getElementById('form1:page1:selectOneChoice7');
	var fechaDesembolso = document
			.getElementById('form1:page1:selectInputDate2');
	var tasaNominalEn = document
			.getElementById('form1:page1:selectOneChoice10');
	...
	if (tasaNominal.value == 1) {
		...
		if (tasaNominal.value == 2) {
			...
			if (tasaNominalVariable == null || tasaNominalVariable.value == 0) {
				...
				if (fechaDesembolso.value == null
						|| fechaDesembolso.value.length == 0) {
...
function validarInformacionIntereses() {
	var tipoPeriodicidadIntereses = document
			.getElementById('form1:page1:selectOneChoice9');
	var periodicidadInteresesPeriodo = document
			.getElementById('form1:page1:selectOneChoice8');
	var periodicidadInteresesDias = document
			.getElementById('form1:page1:inputText11');
	...
	if (tipoPeriodicidadIntereses.value == 0
			|| tipoPeriodicidadIntereses.value == null) {
		mensaje += "- El tipo de periodicidad de los intereses es requerida.\n";
	} else {
		// Si selecciono la periodicida de los intereses, se determina si es
		// periodo o dias y si selecciono o ingreso el valor de la periodicidad.
		// Tipo de periodicidad periodo.
		if (tipoPeriodicidadIntereses.value == 1) {
			// Se valida si selecciono la periodicidad de los intereses.
			if (periodicidadInteresesPeriodo.value == null
					|| periodicidadInteresesPeriodo.value == 0) {
			...
			if (tipoPeriodicidadIntereses.value == 2) {
				...
				if (periodicidadInteresesDias.value == null
						|| periodicidadInteresesDias.value.length == 0) {
...
function formateafechaForm1(obj) {
	...
	with (event) {
		...
		/*
		 * if(keyCode==17 && keyCode==16 && keyCode==188){ alert('dfsdaf');
		 * obj.value= new Date(); return false; }
		 */
		if (ctrlKey && shiftKey && keyCode == 188) {
			...
			// obj.value= fecha.getDate() + "/" + (fecha.getMonth()+1) + "/" +
			// fecha.getFullYear();
			return false;
		}

		if (keyCode == 35 || keyCode == 36 || keyCode == 37 || keyCode == 38
				|| keyCode == 39 || keyCode == 40 || keyCode == 8)
			return false;
...
function bloquearPantalla() {
	...
	// document.getElementById("bloqueador").style.background =
	// "url(../../images/transp.png) repeat";
...
function BloqueoAlphaNum(e) {
	...
	return (keyunicode >= 65 && keyunicode <= 122 || keyunicode == 8 || keyunicode == 32) ? true
			: false

}

/*
 * funcion: Establece el foco para los controles Parametros: id : la propiedad
 * id de componente Autor: Luis Alberto Bola�os Email: labs.sistemas@gmail.com
 */
...
/*
 * funcion: Bloquea el acceso de alpha en campos input Parametros: evt : evento
 * Autor: Luis Alberto Bola�os Email: labs.sistemas@gmail.com
 */
function onlyNumbers(obj) {
	...
	// return (keyunicode>=65 && keyunicode<=122 || keyunicode==8 ||
	// keyunicode==32)? true : false
}
...
/**
 * Funcion encargada de asignar el numero de cedula digitado en la pantalla
 * dinamica a un campo oculto para poder realizar la consulta de los datos del
 * girador.
 */
...
function validarFormularioSolicitudes() {
	...
	if (camposRequeridos.length > 0) {
		for (i = 0; i < camposRequeridos.length; ++i) {
			...
			if (tipoComponente == 2 || tipoComponente == 3
					|| tipoComponente == 12 || tipoComponente == 13) {
	...
	if (!enviarPeticion) {
		...
		$(document).ready(
				function() {
					mostrarMensaje('Hay campos requeridos sin valor', 'ALERTA',
							'alerta.png');
				});
	}
...
/**
 * Metodo encargado de eliminar espacios en blanco a la izquierda y derecha de
 * una cadena.
 * 
 * @param myString
 */
...
function validarEmail(email, id, idBotonCrear, idBotonGuardarParcial,
		idBotonModificar) {
	...
	span
			.setAttribute(
					"style",
					"font-family:Arial,Helvetica,Geneva,sans-serif;font-size:8pt;font-weight:normal;color:#cc0000");
	...
	if (cadena != null && cadena != "" && cadena.length > 0) {
		...
		if (valido) {
			if (document.getElementById("celdaErrorEmail" + id) != null) {
				document.getElementById("celdaErrorEmail" + id).parentNode
						.removeChild(document.getElementById("celdaErrorEmail"
								+ id));
			}
			...
			if (object != null
					&& document.getElementById("celdaErrorEmail" + id) == null) {
		...
		if (document.getElementById("celdaErrorEmail" + id) != null) {
			document.getElementById("celdaErrorEmail" + id).parentNode
					.removeChild(document
							.getElementById("celdaErrorEmail" + id));
		}
...
/* Funciones Jimmy Ramirez */
function validarSeleccion() {
	var combo = document.getElementById('frm-search:selAgente_label');
	var mensajeError = "Debes seleccionar un agente para ser retroalimentado";
...
function aceptarDialogo(contenedor) {
	var res = contenedor.split("j_idt");
	var res2 = contenedor.split(":");
	var num = res[1] - 1;
	var elemento = "";
	for (ciclo = 0; ciclo <= res2[2]; ciclo++) {
		elemento = res2[0] + ":" + res2[1] + ":" + ciclo + ":"
				+ "DialogoConfirmacion";
		var element = document.getElementById(elemento);
		element.parentNode.removeChild(element);
	}
}
function conteoCaracteres(event) {
	var str = document.getElementById("frm-feedback:obsfeedback").value;
	var conteoCaracteres = document.getElementById("limiteTexto");
	var numcar = 0;
	var boton = document.getElementById("frm-feedback:confirmfeed");
	var unicode = ((document.all) ? event.keyCode : event.which);

	if (str.length > 1998) {
		if ((unicode > 48) || (unicode < 90)) {
			event.returnValue = false;
			conteoCaracteres.innerHTML = "Ha llegado al límite de caracteres permitidos";
			boton.style.display = "none";
		}
	} else {
		numcar = 1998 - str.length;
		conteoCaracteres.innerHTML = "Caracteres máximos " + numcar;
		boton.style.display = "";
	}
}
/* Fin funciones Jimmy Ramirez */

/* Inicio Funciones John Pantoja */
